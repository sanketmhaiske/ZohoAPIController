...
describe('Zoho CRM', function () {
  ...
  describe('Zoho Build Data', function () {
    ...
    it('should build XML data from object with wrapper', function () {
      var xml = zohoCRM._build('Cases', this.obj);
  ...
  describe('Create Zoho CRM records', function () {
    ...
    it('should fail when trying to create a cases without params', function () {
      this.timeout(5000);
    ...
    it('should create a cases', function (done) {
      this.timeout(5000);
    ...
    it('should create multiple cases', function (done) {
      this.timeout(5000);
  ...
  describe('Get Zoho CRM records', function () {
    ...
    it('should get cases with params argument', function (done) {
      this.timeout(5000);
      zohoCRM.getRecords(recordType[0], {}, function (error, response) {
        assert.equal(error, null);
        assert.equal(typeof response, 'object');
        assert.equal(response.code, 0);
        assert(Array.isArray(response.data.Cases.row));
        done();
      });

    });

    it('should get cases without params argument', function (done) {
      this.timeout(5000);
      zohoCRM.getRecords(recordType[0], function (error, response) {
        assert.equal(error, null);
        assert.equal(typeof response, 'object');
        assert.equal(response.code, 0);
        assert.equal(typeof response.data.Cases.row, 'object');
        done();
      });
    });

    it('should fail when trying get a cases without id property', function () {
      this.timeout(5000);
    ...
    it('should get a cases by id', function (done) {
      this.timeout(5000);
      zohoCRM.getRecordById(recordType[0], {
        id: created_id
      }, function (error, response) {
        ...
        assert.equal(typeof response, 'object');
        assert.equal(response.code, 0);
        assert(response.data.Cases.row);
        done();
      });
    });
  });
  ...
  describe('Update Zoho CRM records', function () {
    ...
    it('should fail when trying update a cases with id param missing', function () {
      this.timeout(5000);
    ...
    it('should update a cases', function (done) {
      this.timeout(5000);
  ...
  describe('Delete Zoho CRM records', function () {
    ...
    it('should fail when trying delete a cases with id param missing', function () {
      this.timeout(5000);
    ...
    it('should delete a cases', function (done) {
      this.timeout(5000);
