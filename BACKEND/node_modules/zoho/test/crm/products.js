...
describe('Zoho CRM', function () {
  ...
  describe('Zoho Build Data', function () {
    ...
    it('should build XML data from object with wrapper', function () {
      ...
      assert(xml.get('row').child(0).text(), this.obj['Product Name']);
    });
  });
  ...
  describe('Create Zoho CRM records', function () {
    ...
    it('should create multiple products', function (done) {
      this.timeout(5000);
  ...
  describe('Get Zoho CRM records', function () {
    ...
    it('should get products with params argument', function (done) {
      this.timeout(5000);
      zohoCRM.getRecords(recordType[0], {}, function (error, response) {
        assert.equal(error, null);
        assert.equal(typeof response, 'object');
        assert.equal(response.code, 0);
        assert(Array.isArray(response.data.Products.row));
        done();
      });

    });

    it('should get products without params argument', function (done) {
      this.timeout(5000);
      zohoCRM.getRecords(recordType[0], function (error, response) {
        assert.equal(error, null);
        assert.equal(typeof response, 'object');
        assert.equal(response.code, 0);
        assert.equal(typeof response.data.Products.row, 'object');
        done();
      });
    });
    ...
    it('should get a contact by id', function (done) {
      ...
      zohoCRM.getRecordById(recordType[0], {
        ...
        assert.equal(typeof response, 'object');
        assert.equal(response.code, 0);
        assert(response.data.Products.row);
        done();
      });
    });
  });
