...
describe('Zoho CRM', function () {
  ...
  describe('Zoho Build Data', function () {
    ...
    it('should build XML data from object with wrapper', function () {
      var xml = zohoCRM._build('SalesOrders', this.obj);
      xml = libxml.parseXml(xml);

      assert(xml.get('row').child(0).text(), this.obj['Subject']);
    });
  });
  ...
  describe('Create Zoho CRM records', function () {
    ...
    it('should fail when trying to create a salesorder without params', function () {
      this.timeout(5000);
    ...
    it('should create a salesorder', function (done) {
      this.timeout(5000);
    ...
    it('should create multiple salesorder', function (done) {
      this.timeout(5000);
  ...
  describe('Get Zoho CRM records', function () {
    ...
    it('should get salesorder with params argument', function (done) {
      this.timeout(5000);
      zohoCRM.getRecords(recordType[0], {}, function (error, response) {
        assert.equal(error, null);
        assert.equal(typeof response, 'object');
        assert.equal(response.code, 0);
        assert(response.data.SalesOrders.row);
        done();
      });

    });

    it('should get salesorder without params argument', function (done) {
      this.timeout(5000);
      zohoCRM.getRecords(recordType[0], function (error, response) {
        assert.equal(error, null);
        assert.equal(typeof response, 'object');
        assert.equal(response.code, 0);
        assert.equal(typeof response.data.SalesOrders.row, 'object');
        done();
      });
    });

    it('should fail when trying get a salesorder without id property', function () {
      this.timeout(5000);
    ...
    it('should get a salesorder by id', function (done) {
      this.timeout(5000);
      zohoCRM.getRecordById(recordType[0], {
        id: created_id
      }, function (error, response) {
  ...
  describe('Update Zoho CRM records', function () {
    ...
    it('should fail when trying update a salesorder with id param missing', function () {
      this.timeout(5000);
    ...
    it('should update a salesorder', function (done) {
      this.timeout(5000);
  ...
  describe('Delete Zoho CRM records', function () {
    ...
    it('should fail when trying delete a salesorder with id param missing', function () {
      this.timeout(5000);
    ...
    it('should delete a salesorder', function (done) {
      this.timeout(5000);
